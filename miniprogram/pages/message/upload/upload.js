...
Page({
  ...
  getData() {
    ...
    wx.request({
      url: `${urls.get_remind}?uid=${app.globalData.cry_id}&type=upload&page=${self.data.page}&per_page=${per_page}`,
      success(res) {
        if (res.data.length && typeof (res.data) == 'object') {
          ...
          res.data.forEach((item, index) => {
            ...
            tempCloudID.push(item.target.iid)
          })
